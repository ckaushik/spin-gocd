...
This project uses Terraform to assign a DNS name to the GoCD server using Route53. See the file [../tree/blob/master/domain.tf](../tree/blob/master/domain.tf) for this - it just needs to be able to create a record. I have a domain I use for this, whose name servers are pointed to the AWS name servers for the zone in Route53. I don't have anything else important under this domain.
